...
void big_integer::binary_operation(big_integer b, std::function<uint32_t(uint32_t, uint32_t)> const& operation) {
    resize(b.length());
    b.resize(length());
    bool tmp_sign = operation(sign, b.sign);
    if (sign)
        additional_code();
    if (b.sign)
        b.additional_code();
    for (size_t i = 0; i < length(); i++) {
        number[i] = operation(number[i], b[i]);
    }
    if (tmp_sign) {
        additional_code();
    }
...
big_integer& big_integer::operator&=(big_integer const& rhs) {
    binary_operation(rhs, [](uint32_t const& a, uint32_t const& b) -> uint32_t {return a & b; });
    return *this;
}
